...
In Roman numerals 1990 is rendered: 1000=M, 900=CM, 90=XC; resulting in MCMXC. 2008 is written as 2000=MM, 8=VIII; or MMVIII. 1666 uses each Roman symbol in descending
order: MDCLXVI.
...
let RomanNumerals = {
  toRoman: function (num) {
    var digits = String(+num).split(""),result = "",i = 3;
    while (i--)
  ...
  fromRoman: function (s) {
    ...
    for (const char of s.split("").reverse()) {
      (values.get(char) >= p) ? r += values.get(char) : r -= values.get(char);
      p = values.get(char);
    }
    return r;
  }
